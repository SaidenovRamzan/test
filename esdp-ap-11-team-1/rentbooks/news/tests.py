...
class NewsModelTests(TestCase):
    ...
    def test_update_news_image(self):
        ...
        new_image = SimpleUploadedFile('test2.jpg', new_image_data_io.read(), content_type='image/jpeg')
        news.image = new_image
        news.save()

        self.assertTrue(News.objects.get(id=news.id).image.name.startswith('images/news/'))
