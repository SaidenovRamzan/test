...
from books.models import Composition, Genres, Book, Comment, FavoriteBook, Rating, FavoriteBook
    ...
    def get(self, request):
        ...
        for composition in compositions:
            if Book.objects.all().filter(id_composition=composition.id):
                coverphoto = Book.objects.all().filter(id_composition=composition.id).first().coverphoto
        ...
        return render(request, template_name, context={context_object_name: composition_objects})
...
class ListOfCompositionsView(View):
    ...
    def get(self, request):
        ...
        if purpose:
            user_shelfs = UserShelf.objects.filter(purpose__in=[purpose, 'rent and sale'])
            book_ids = [user_shelf.id_book for user_shelf in user_shelfs]
            purpose_books = Book.objects.filter(id_Book__in=book_ids)
            purpose_compositions_ids = [purpose_book.id_composition.id for purpose_book in purpose_books]
            compositions = compositions.filter(id_Composition__in=purpose_compositions_ids)
        if language:
        ...
        for composition in sorted_compositions:
            if Book.objects.all().filter(id_composition=composition.id):
                coverphoto = base64.b64encode(
                    Book.objects.all().filter(id_composition=composition.id).first().coverphoto.read()).decode('utf-8')
            else:
                coverphoto = None
        ...
        for user_shelf in user_shelfs:
            user_shelf_objects.append({
                ...
                'coverphoto': base64.b64encode(
                    Book.objects.get(id_Book=user_shelf.id_book).coverphoto.read()).decode(
                    'utf-8'),
    ...
    def sort_by_recommendation_data(self, compositions):
        ...
        if len(favorites) >= 5:
            ...
            other_compositions = [comp for comp in compositions if comp not in favorite_compositions]
        else:
            favorite_compositions = Composition.objects.none()
            other_compositions = compositions

        ratings = Rating.objects.values('composition_id').annotate(avg_rating=Avg('rating'))
        for comp in favorite_compositions:
            comp.avg_rating = next((r['avg_rating'] for r in ratings if r['composition_id'] == comp.id_Composition), 0)
        sorted_favorite_compositions = sorted(favorite_compositions, key=lambda x: (x.avg_rating, x.rent_qte),
                                              reverse=True)

        # Сортировка по рейтингу и rent_qte для остальных композиций
        for comp in other_compositions:
            comp.avg_rating = next((r['avg_rating'] for r in ratings if r['composition_id'] == comp.id_Composition), 0)
        sorted_other_compositions = sorted(other_compositions, key=lambda x: (x.avg_rating, x.rent_qte), reverse=True)
...
class CompositionCreateView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            # composition.save()
            return redirect('home')
        return render(request, self.template_name, context={'forms': self.form, 'errors': self.form.errors})
...
class CompositionDetailView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        comments = Comment.objects.filter(composition_id=kwargs['pk']).order_by('-date_publish')
        ...
        try:
            coverphoto_base64 = base64.b64encode(books.first().coverphoto.read()).decode('utf-8')
        except:
            coverphoto_base64 = None

        # Достаем все объявления, привязанные к композиции
        user_shelfs = UserShelf.objects.all().filter(id_book__in=[id_book.id for id_book in books], is_active=True)
        user_shelf_objects = []

        for user_shelf in user_shelfs:
            # Достаем объявление, которое привязано к композиции
            if OrderOfRent.objects.filter(user=request.user, usershelf=user_shelf, is_active=True, is_finished=False).exists():
                order = OrderOfRent.objects.filter(user=request.user, usershelf=user_shelf, is_active=True, is_finished=False).first()
            else:
                order = None
            user_shelf_objects.append({
                'user_shelf': user_shelf,
                'book': Book.objects.get(id_Book=user_shelf.id_book),
                'coverphoto': base64.b64encode(Book.objects.get(id_Book=user_shelf.id_book).coverphoto.read()).decode(
                    'utf-8'),
                'order': order,
                'coordinates': list(map(float, self.get_coordinates(user_shelf)))
            })
        if self.request.user.id:
            rating_obj = Rating.objects.filter(user=self.request.user,
                                               composition_id=composition.id_Composition).first()
            rating = rating_obj.rating if rating_obj else 0
        else:
            rating = 0
        avg_rating = Rating.objects.filter(composition_id=composition.id_Composition).aggregate(
...
class CompositionAdminListView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, 'admin_compositions.html', context={'compositions': compositions})