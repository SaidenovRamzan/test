...
class AddBookView(View):
    def get(self, request):
        ...
        return render(
            request, "book__detail.html", context={"forms": form, "book": obj}
        )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            book.save()
            return render(
                request, "book__detail.html", context={"forms": form}
            )  # Перенаправление на страницу успешного добавления книги
...
class BookView(View):
    def get(self, request):
        form = BookForm()
        form.fields["id_Composition"].choices = [
    ...
    def post(self, request):
        ...
        form.fields.get("id_Composition").choices = [
            (composition.id, composition.name)
            for composition in Composition.objects.all()
        ]
        if form.is_valid():
            book = Book(
                id_composition=Composition.objects.get(
                    id_Composition=form.cleaned_data["id_Composition"]
                ),
...
class BookCreateView(View):
    ...
    def get(self, request, *args, **kwargs):
        self.form.fields["id_Composition"].choices = [
            (composition.id, composition.name)
            for composition in Composition.objects.filter(is_visible=True)
        ]
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return redirect("home")
        return render(
            request, "create_book.html", context={"forms": form, "errors": form.errors}
        )
...
class BookUpdateView(View):
    ...
    def get(self, request, *args, **kwargs):
        book = self.model.objects.get(id_Book=kwargs["pk"])
        return render(
            request, self.template_name, context={"form": self.form_class, "book": book}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            book.id_composition = Composition.objects.get(
                id_Composition=form.cleaned_data["id_Composition"]
            )
...
class BookImageViewSet(viewsets.ModelViewSet):  # добавить
    ...
    def list(self, request, *args, **kwargs):
        ...
        for ushf in UserShelf.objects.all():  # перебираем все объявления
            if (
                ushf.user.id == curr_user.id
            ):  # если объявление принадлежит текущему пользователю,
    ...
    def create(self, request, *args, **kwargs):
        id_us = int(
            request.data["book_shelf"]
        )  # получаем id объявления {'book_shelf': ['3']}
        us = UserShelf.objects.get(pk=id_us)  # получаем объект объявления по id_us
        book_id = us.id_book

        files = request.FILES.getlist(
            "image"
        )  # получаем по ключу image список добавленных объектов картинок
        for file in files:  # перебираем список объектов картинок
            ...
            )  # добавляем список с одним объектом картинки в сериализатор
            serializer.is_valid(
                raise_exception=True
            )  # проверка валидности сериализатора
            self.perform_create(
                serializer, id_us, book_id
            )  # передаем id объявления (id usershelf) на функцию создания
    ...
    def perform_create(
        self, serializer, id_us, book_id
    ):  # функция создания новой записи с картинкой в UserImages
