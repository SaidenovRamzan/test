    ...
    def get(self, request, *args, **kwargs):
        ...
        response = {
            ...
            "coverphoto": base64.b64encode(
                Book.objects.all()
                .filter(id_composition=object.id)
                .first()
                .coverphoto.read()
            ).decode("utf-8"),
        }
...
class AddToFavorites(APIView):
    def post(self, request, *args, **kwargs):
        ...
        is_favorite = FavoriteBook.objects.filter(
            user_id=user_id, composition_id=composition_id
        ).exists()
...
class GetBookPublishHouses(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        publish_houses = [
            {"publish_house": izdatelstvo} for izdatelstvo in unique_publish_houses
        ]
        return JsonResponse(
            {"publish_houses": publish_houses}, status=status.HTTP_200_OK
        )
...
class GetBookYearByPublishHouse(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            books = (
                Book.objects.all()
                .filter(id_composition=object.id_Composition)
                .filter(izdatelstvo=izdatelstvo)
            )
...
class GetBookISBNByYear(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if book:
                return JsonResponse(
                    {"isbn": book.isbn, "bookId": book.id}, status=status.HTTP_200_OK
                )
            else:
                return JsonResponse(
                    {"error": "Invalid JSON"}, status=status.HTTP_404_NOT_FOUND
                )
        except json.JSONDecodeError:
            return JsonResponse(
                {"error": "Invalid JSON"}, status=status.HTTP_400_BAD_REQUEST
...
class AddCommentToCompositon(APIView):
    def post(self, request, *args, **kwargs):
        ...
        comment = Comment.objects.create(
            user_id=user_id, composition_id=composition_id, text=text
        )
        return JsonResponse({"id": comment.id, "text": comment.text}, status=200)
...
class ListCommentsToCompositon(APIView):
    def get(self, request, *args, **kwargs):
        comments = Comment.objects.filter(composition_id=kwargs.get("pk")).order_by(
            "-date_publish"
        )
...
class AddRatingToCompositon(APIView):
    def post(self, request, *args, **kwargs):
        ...
        rating_obj, created = Rating.objects.get_or_create(
            user_id=user_id, composition_id=composition_id, defaults={"rating": rating}
        )
        if not created:
...
class CompositionSelectedVisible(APIView):
    def post(self, request, *args, **kwargs):
        ...
        for compostion in compositions:
            ...
            Composition.objects.filter(id_Composition=composition_id).update(
                is_visible=True
            )
